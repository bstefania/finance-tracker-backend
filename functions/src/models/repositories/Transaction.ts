...
const constructQuery = async (collection: string, queryParams: any, userRef: any) => {
  ...
  if (queryParams.categoryGroup) {
    const groupsSnapshot = await db
      .collection(categoryGroupsCollectionName)
      .where("owner", "==", userRef)
      .where("name", "==", queryParams.categoryGroup)
      .get();
  ...
  if (queryParams.category) {
    ...
    if (queryParams.categoryGroup) {
      const categorySnapshot = await db
        ...
        .where("owner", "==", userRef)
        .where("categoryGroup", "in", groupDocs)
        .get();
      categoryDocs = querySnapshotToRef(categorySnapshot);
      if (!categoryDocs.length) {
        return;
...
export const getTransactions = async (userId: string, queryParams: Record<string, string>) => {
  ...
  return Promise.all(
    querySnapshot.docs.map(async (ref) => {
      const data = ref.data();
      return {
        id: ref.id,
        type: data.type,
...
export const getTransaction = async (transactionId: string, userId: string) => {
  // use one function
  const transactionRef = (await idsToRef(
    transactionId,
    collectionName
  )) as DocumentReference<FirebaseFirestore.DocumentData>;
...
export const createTransaction = async (transaction: TransactionInput, userId: string) => {
  ...
  const transactionRef = await db.collection(collectionName).add(newData);

  if (!transactionRef) {
    throw new ApiError(HttpResponse.INTERNAL_SERVER_ERROR, "Transaction could not be created.");
  }

  await updateWealth(user, userRef, newData.type, newData.source, newData.amount);
...
export const updateTransaction = async (
  ...
  const oldTransaction = (await refsToData(transactionRef, collectionName)) as unknown as Transaction;

  checkAccess(oldTransaction, userId, collectionName);
  ...
  if (transactionUpdates.categoryId) {
    ...
    const categoryData = (await refsToData(newData.category, collectionName)) as unknown as Transaction;
    checkAccess(categoryData, userId, categoriesCollectionName);
  }
  ...
  const updatedTransaction = (await refsToData(transactionRef, collectionName)) as unknown as Transaction;
...
export const deleteTransaction = async (transactionId: string, userId: string) => {
  ...
  if (!res) {
    throw new ApiError(HttpResponse.INTERNAL_SERVER_ERROR, "Transaction could not be deleted.");
  }
  const userRef = (await idsToRef(userId, usersCollectionName)) as DocumentReference;
  const user = (await refsToData(userRef, usersCollectionName)) as unknown as User;

  await updateWealth(user, userRef, transaction.type, transaction.source, transaction.amount, true);
...
const updateWealth = async (
  ...
  if (!amount || !transactionType || !transactionSource) {
    throw new ApiError(HttpResponse.INTERNAL_SERVER_ERROR, "Wealth couldn't be updated. Missing data.");
  }
  if (undo) {
    amount = -amount;
  }
